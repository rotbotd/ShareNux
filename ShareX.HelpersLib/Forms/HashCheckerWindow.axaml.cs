...
namespace ShareX.HelpersLib
{
    public partial class HashCheckerWindow : Window
    {
        ...
        private void CbCompareTwoFiles_CheckedChanged(object? sender, RoutedEventArgs e)
        {
            ...
            lblFilePath2.IsVisible = compare;
            txtFilePath2.IsVisible = compare;
            btnFilePathBrowse2.IsVisible = compare;
            txtTarget.IsVisible = !compare;
            
            // Find the target label and hide it too
            var targetLabel = this.FindControl<TextBlock>("lblTarget");
            if (targetLabel != null) targetLabel.IsVisible = !compare;
        }
        ...
        private async void BtnStartHashCheck_Click(object? sender, RoutedEventArgs e)
        {
            ...
            {
                isWorking = false;
                btnStartHashCheck.Content = "Start";
        ...
        private void UpdateResultColor()
        {
            ...
            {
                txtResult.Foreground = Avalonia.Media.Brushes.Black;
            }
            else if (result.Equals(target, StringComparison.OrdinalIgnoreCase))
            {
                txtResult.Foreground = Avalonia.Media.Brushes.Green;
            }
            else
            {
